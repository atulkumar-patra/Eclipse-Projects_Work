...
import com.utils.displaybookshelves.extentReportManager;
import com.utils.displaybookshelves.dateUtil;
...
public class baseClass {
	...
	//Slider
	//factor decides x-axis & y-axis movement
	//Upper Slider
	public void Uslider(String xpathKey, int factor1, int factor2) {
		Actions action = new Actions(driver);
		WebElement element = null;
		element = getElement(xpathKey);
		int uwidth = element.getSize().width;
		action.dragAndDropBy(element, (int) (-uwidth*(factor1)), factor2);  
		action.build().perform();
		
	}
	
	//Lower Slider
	public void Lslider(String xpathKey, int factor1, int factor2) {
		Actions action = new Actions(driver);
		WebElement element = null;
		element = getElement(xpathKey);
		int lwidth = element.getSize().width;
		action.dragAndDropBy(element, (int) (lwidth*(factor1)), factor2);   
		action.build().perform();
